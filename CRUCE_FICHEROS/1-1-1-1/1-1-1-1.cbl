	...
	*>*************************************************
	...
	FILE-CONTROL.
		...
		SELECT FITXER4 ASSIGN TO "FITXER4.TXT" STATUS IS FS-FITXER4.
	...
	FD FITXER4
		DATA RECORD IS REG-FITXER4.
		01 REG-FITXER4.
			05 REG-ID4	 	PIC 9(3).
			05 REG-CNOM 	PIC X(10).				
	FD SORTIDA
	...
	01 MISATGES.
		05 MSG-IGUAL.
			...
			10 FILLER PIC X(9) VALUE "  CNOM-->".
			10 MSG-IGUAL-CNOM PIC X(1).
	...
	PERFORM LLEGIR-FITXER4 THRU FINAL-LLEGIR-FITXER4
	...
	PERFORM UNTIL FINAL-FITXER1
		...
		EVALUATE TRUE
			...
			WHEN ID1 > ID2 AND ID3 > ID2
				PERFORM LLEGIR-FITXER2				
			WHEN ID1 < ID2 AND ID1 < ID3
				PERFORM LLEGIR-FITXER1
			WHEN ID3 < ID2 AND ID4 > ID3
				PERFORM LLEGIR-FITXER3
			WHEN ID3 > ID2 AND ID4 > ID2
				PERFORM LLEGIR-FITXER2
			WHEN ID3 < ID4 AND ID1 > ID3
				PERFORM LLEGIR-FITXER3
			WHEN ID3 > ID4 AND ID2 > ID4
				PERFORM LLEGIR-FITXER4
			
			WHEN OTHER
				DISPLAY "FALTAN CONDICIONS AL WHEN"
	...
	READ FITXER4 INTO WX-FITXER4

	EVALUATE FS-FITXER4
		WHEN ZEROES
			MOVE WX-ID4 TO ID4
			MOVE WX-CNOM TO CNOM
		WHEN 10
			SET FINAL-FITXER4 TO TRUE
			MOVE HIGH-VALUES TO ID4
		WHEN OTHER
			PERFORM PROCES-FINAL
	END-EVALUATE.

	FINAL-LLEGIR-FITXER4.EXIT.


	PROCES-FINAL.

	CLOSE FITXER1 FITXER2 FITXER3 FITXER4 SORTIDA
